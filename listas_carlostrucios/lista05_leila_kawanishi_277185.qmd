---
title: "lista05_leila_kawanishi_277185"
...
SELECT PNAME,
      ACRES,
      ACRES + (ACRES * 0.2) AS INCREASED_ACRES
FROM PRESERVE
...
SELECT PNAME, 
       ACRES,
       ACRES - (ACRES / 3) AS DECREASED_ACRES
FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(FEE + (ACRES * 0.02)) AS AJUSTED_AVG_FEE
FROM PRESERVE
...
SELECT ENO,
       ENAME,
       SALARY AS OLDSALARY,
       SALARY + (SALARY * 0.1) AS NEWSALARY
FROM EMPLOYEE
ORDER BY NEWSALARY

...
table1
```
Resposta: Sim, o banco table1 está em formato tidy, pois as colunas representam variaveis e as linhas contém apenas uma observação para cada variável
...
Resposta: Sim, o banco table2 está em formato tidy, pois as colunas representam variaveis e as linhas contém apenas uma observação para cada variável

```{r}
table3
```

Resposta: Não, o banco table3 não está em formato tidy, pois a coluna rate esta classificada como <chr> e possue duas inforamções em uma única variavel, embora numericamente ela possa comunicar uma taxa, as informações contidas na coluna são referentes ao "cases" e a "population".

```{r}
table4a
```

Resposta: Não, aqui temos duas colunas cominucando data, o formato longo (ou tidy) seria formado por uma tabela contendo país, ano e número de casos
